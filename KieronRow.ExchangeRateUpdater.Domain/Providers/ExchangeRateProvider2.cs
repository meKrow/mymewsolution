...
{
    public class ExchangeRateProvider2 : BaseRateProvider
    {
        const string fromCurrency = "CZK";

        public ExchangeRateProvider2(IApiRepo exchangeRateApiRepo) : base(exchangeRateApiRepo) {}
        ...
        public override async Task<List<ExchangeRate>> GetExchangeRates(IEnumerable<Currency> currencies)
        {
            if (ValidateCurrencies(currencies))
            {
                ...
                if (apiRates != null && apiRates.rates.Any())
                {
                    ...
                    return filteredExchangeRates.OrderBy(x => x.TargetCurrency.Code).Take(1).ToList();
                }
            }
            else
            {
